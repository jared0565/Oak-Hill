...
import { hashToken, permissionsFor, mustEnroll2fa } from "../_lib/auth-core.mjs";
...
export async function onRequestGet(ctx) {
  ...
  return Response.json({ mustEnroll2fa: mustEnroll2fa({ role: sess.role, totp_enabled: sess.totp_enabled }), user: { name: sess.name, email: sess.email, role: sess.role, permissions: permissionsFor(sess.role), avatar: sess.avatar || null, totp_enabled: !!sess.totp_enabled } });
}
